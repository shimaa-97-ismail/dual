...
import { useDeleteSpecial, useUpdateSpecial } from "../../hooks/useSpecial";
import {
  Card,
  CardContent,
  CardFooter,
...
export function SpecialCard({ data ,onSpecialClick}) {
  const updateMutation = useUpdateSpecial();
  const deleteMutation = useDeleteSpecial();
      ...
      <div className="w-full hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
        <Card className="h-full flex flex-col border-2 hover:border-primary/20">
          <CardHeader
            ...
            onClick={() => {
              onSpecialClick(data._id,data.name)
          ...
          <CardContent className="flex items-center justify-between">
            <div className="mb-4">
              <h3 className="text-lg font-semibold mb-2 text-gray-700" > عدد المدارس : {data.schoolCount}</h3>
            </div>
          </CardContent>
