...
export const getAllTrainningPlaces = async (req, res) => {
  try {
    ...
    const trainningPlaces = await TrainningPlace.find(query)
      .populate("supervisorName", "username phone")
...
export const getTrainningPlaceById = async (req, res) => {
  try {
    const { id } = req.params;
    const trainningPlace = await TrainningPlace.findById(id).populate(
      "supervisorName",
      "username role",
    );
    if (!trainningPlace) {
...
export const deleteTrainningPlace = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Training Place deleted successfully" });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
